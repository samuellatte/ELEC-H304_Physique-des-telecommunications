...
# On définit ensuite des matrices appelables par les process
Puissance_dBm = mp.Array('d', int(largeur_avion / resol_y) * int(longueur_avion / resol_x))
Puissance_dBm = np.frombuffer(Puissance_dBm.get_obj())
Puissance_dBm = Puissance_dBm.reshape((int(largeur_avion / resol_y), int(longueur_avion / resol_x)))  # b and arr share the same memory
Debit_binaire = mp.Array('d', int(largeur_avion / resol_y) * int(longueur_avion / resol_x))
Debit_binaire = np.frombuffer(Debit_binaire.get_obj())
Debit_binaire = Debit_binaire.reshape((int(largeur_avion / resol_y), int(longueur_avion / resol_x)))  # b and arr share the same memory

# Ainsi que des matrices formatées pour le script dessin
dBm = []
binaire = []
...
def calc_param(): # Calcule, pour chaque mur, les paramètres correspondants
    for indice,cle in enumerate(murs):
        ...
        if indice <= 19: # On ajoute les paramètres GRP
        ...
        else: # On ajoute les paramètres plastique
...
def verif_transmission(TX,RX): # Vérifie sur le rayon traverse 1/des mur(s)
    T_m = 1
        ...
        if murs[cle][2][0] * d[1] != murs[cle][2][1] * d[0]:
            ...
            if 0 < t < np.linalg.norm(murs[cle][1] - murs[cle][0]):  # Vérifie s'il y a une transmission
                if np.sign(np.dot(s, murs[cle][3])) != np.sign(np.dot(RX - murs[cle][0], murs[cle][3])) and np.sign(np.dot(RX - murs[cle][0], murs[cle][3])) != 0 and np.sign(np.dot(s, murs[cle][3])) != 0:
                    ...
                    # print('Pt = ', P)
                    T_m = calc_coeff_transmission(d,cle,T_m) # Appelle la fonction qui calcule le coefficient de transmission
    return T_m


def calc_coeff_transmission(d,cle,T_m): # Calcul du coefficient de tranmsission
...
def calc_coeff_reflexion(d,cle): # calcule le coefficient de rélfexion
    norm_d = np.linalg.norm(d)
    d_unit = d/norm_d
    cos_theta_i = np.linalg.norm(np.dot(d_unit, murs[cle][3]))
    sin_theta_i = np.sqrt(1 - cos_theta_i ** 2)
...
def trajet_direct(TX,RX): # Calcule le trajet direct
    if np.array_equal(TX,np.array([19,1.5])):
        T_m = verif_transmission(TX,RX)
        d = [RX[0] - TX[0], RX[1] - TX[1]]  # On définit le vecteur d
        norm_d = np.linalg.norm(d)
        E0 = T_m * np.sqrt(60 * PtxGtx) * (np.exp(-beta * norm_d * 1j) / (norm_d))
        P_RX_Watt = ((lambd ** 2) / (8 * (constants.pi ** 2) * R)) * np.linalg.norm(E0) ** 2  # Puissance reçue grâce à l'onde direct, en [W]
...
def calc_image(TX,RX,nbr_rebond): # Calcule toutes les images et puissances associées
    P_RX_Watt_1 = 0
    P_RX_Watt_2 = 0
    P_RX_Watt_3 = 0
            ...
            for cle in murs:
                ...
                if murs[cle][2][0] * d1[1] != murs[cle][2][1] * d1[0]: # Condition pour éviter que t1 soit infini
                    ...
                    if 0 < t1 < np.linalg.norm(murs[cle][1] - murs[cle][0]) and np.sign(np.dot(s1, murs[cle][3])) == np.sign(np.dot(RX - murs[cle][0], murs[cle][3])): # Si t1 est dans le mur et que la condition avec s et n est vérifiée
                        ...
                        P_RX_Watt_1 += ((lambd ** 2) / (8 * (constants.pi ** 2) * R)) * np.linalg.norm(E1) ** 2  # Puissance reçue grâce à l'onde direct, en [mW]
                                        ...
                                        if 0 < t2 < np.linalg.norm(murs[cle2][1] - murs[cle2][0]) and np.sign(np.dot(s2, murs[cle2][3])) == np.sign(np.dot(RX - murs[cle2][0], murs[cle2][3])):  # Si t2 est dans le mur et que la condition avec s et n est vérifiée
                                            ...
                                            if murs[cle][2][0] * d1[1] != murs[cle][2][1] * d1[0]:
                                                ...
                                                if 0 < t1 < np.linalg.norm(murs[cle][1] - murs[cle][0]):
                                                    ...
                                                    P_RX_Watt_2 += ((lambd ** 2) / (8 * (constants.pi ** 2) * R)) * np.linalg.norm(E2) ** 2  # Puissance reçue grâce à l'onde direct, en [mW]
                                                                    ...
                                                                    if 0 < t2 < np.linalg.norm(murs[cle2][1] - murs[cle2][0]):
                                                                        ...
                                                                        if murs[cle][2][0] * d1[1] != murs[cle][2][1] * d1[0]:
                                                                            ...
                                                                            if 0 < t1 < np.linalg.norm(murs[cle][1] - murs[cle][0]):
                                                                                ...
                                                                                P_RX_Watt_3 += ((lambd ** 2) / (8 * (constants.pi ** 2) * R)) * np.linalg.norm(E3) ** 2  # Puissance reçue grâce à l'onde direct, en [mW]
...
def puissances(j,Puissances_dBm,Debit_binaire,ligne): # Fonction appelée par chaque process, calcule la puissance sur toutes les lignes en parallèle
    col = 0
    for i in np.arange(resol_x / 2, longueur_avion + (resol_x / 2), resol_x):  # On itère sur les colonnes
        RX = [i, j]
        # print(RX)
        P_RX_Watt = 0
        ...
        # print(P_RX_dBm)
        # print(Deb_bin)
        Puissances_dBm[ligne][col] = P_RX_dBm
        Debit_binaire[ligne][col] = Deb_bin
        # print(ligne)
        # print(col)
        col += 1
    # print(Puissances_dBm)
    # print(Debit_binaire)


def main(): # Fonction main, lance le calcul
    start = time.time()
    ...
    processes = []
    ligne = 0
    for j in np.arange(largeur_avion - resol_y / 2, -resol_y / 2,-resol_y): # On itère sur les lignes, de resol_y/2 à largeur_avion-(resol_y)/2
        p = mp.Process(target=puissances, args=(j, Puissance_dBm, Debit_binaire, ligne))
        processes.append(p)
        p.start()
        ligne +=1

    for elem in processes: # Permet d'attendre que les process aient fini
        elem.join()

    for ligne in range(int(largeur_avion/resol_y)): # Ajout des valeurs d'une matrice à l'autre afin d'avoir le bon format
        for elem in Puissance_dBm[ligne, :]:
            dBm.append(elem)
        for elem in Debit_binaire[ligne,:]:
            binaire.append(elem)

    end = time.time()
    print('Temps exécution =', end - start, 'secondes')
    
    # Partie sauvegarde des données :
    if nbr_rebond == 0:
        np.savetxt('P_RX_dBm-0_thread_mat', dBm)
        np.savetxt('Debit_bin-0_thread_pos_mat', binaire)
    if nbr_rebond == 1:
        np.savetxt('P_RX_dBm-1_thread_mat', dBm)
        np.savetxt('Debit_bin-1_thread_mat', binaire)
    if nbr_rebond == 2:
        np.savetxt('P_RX_dBm-2_thread_mat', dBm)
        np.savetxt('Debit_bin-2_thread_mat', binaire)
    if nbr_rebond == 3:
        np.savetxt('P_RX_dBm-3_thread_mat', dBm)
        np.savetxt('Debit_bin-3_thread_mat', binaire)
main()